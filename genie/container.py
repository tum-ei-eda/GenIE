...
## This file is internal to GenIE and is not part of the API.
import os
import re
import uuid
import shlex
...
def gui_args(osinfo: OSInfo) -> List[str]:
    ...
    if osinfo.kernel == "Linux":
        if os.environ.get("DISPLAY") is None:
            warn("DISPLAY environment variable not set. GUI features will not be available.")
        else:
            args += [
                "-e",
...
def remote_manifest_exists(image: str) -> bool:
    ...
    try:
        httpx.Client(follow_redirects=True).get(url, headers={"Accept": "application/json"})
...
def run_in_container(
    ...
    if not osinfo.supported:
        warn(f"Unsupported host operating system '{osinfo.kernel}'. You may encounter unexpected issues.")
    ...
    if osinfo.container_info.engine.lower() == "docker":
        ...
        warn(f"Unsupported container engine '{osinfo.container_info.engine}'. You may encounter unexpected issues.")
    ...
    tempdir = tempfile.mkdtemp("genie_docker")
