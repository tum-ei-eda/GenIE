...
from openlane.state.design_format import (
    DesignFormat,
    DesignFormatObject,
)
...
class State(GenericImmutableDict[str, StateElement]):
    ...
    def __init__(
        self,
        copying: Optional[Union[Mapping[str, StateElement], Mapping[DesignFormat, StateElement]]] = None,
        *args,
        overrides: Optional[Union[Mapping[str, StateElement], Mapping[DesignFormat, StateElement]]] = None,
        ...
        if o_mapping := overrides:
            for k, value in o_mapping.items():
                if isinstance(k, DesignFormat):
                    assert isinstance(k.value, DesignFormatObject)  # type checker shut up
                    k = k.value.id
                overrides_resolved[k] = value

        self.metrics = GenericImmutableDict(metrics or {})
        self.paths = GenericImmutableDict(paths or {})
    ...
    def to_raw_dict(self, metrics: bool = True, paths: bool = True) -> Dict[str, Any]:
        final = super().to_raw_dict()
        if metrics:
            final["metrics"] = self.metrics.to_raw_dict()
        if paths:
            final["paths"] = self.paths.to_raw_dict()
        return final

    def copy(self: "State") -> "State":
        metrics: GenericImmutableDict[str, Any] = GenericImmutableDict(copy_recursive(self.metrics))
        paths: GenericImmutableDict[str, Any] = GenericImmutableDict(copy_recursive(self.paths))
        new = State(self, metrics=metrics, paths=paths)
        return new
    ...
    def save_snapshot(self, path: Union[str, os.PathLike]):
        ...
        paths_json_path = os.path.join(path, "paths.json")
        with open(paths_json_path, "w", encoding="utf8") as f:
            f.write(self.paths.dumps())
    ...
    def validate(self):
        ...
        def visitor(key, value, top_key, _, depth):
            if depth == 0 and DesignFormat.by_id(top_key) is None:
                pass
                raise InvalidState(f"Key '{top_key}' does not match a known design format.")
            if value is None:
                return
            if not (isinstance(value, Path) or isinstance(value, dict) or isinstance(value, list)):
                raise InvalidState(f"Value at '{key}' is not a Path nor a dictionary/list of Paths: '{value}'.")

        self._walk(self.to_raw_dict(metrics=False, paths=False), "", visit=visitor)
        ...
        for key, value in views.items():
            ...
            if isinstance(value, dict):
                ...
                if validate_path and not os.path.exists(value):
                    raise ValueError(f"Provided path '{value}' to design format '{current_key_path}' does not exist.")
    ...
    def loads(Self, json_in: str, validate_path: bool = True) -> "State":
        ...
        state = Self(views, metrics=metrics, paths=paths)
    ...
    def _repr_html_(self) -> str:
        return (
            ...
            + self.__mapping_to_html_rec(
                self.to_raw_dict(metrics=False, paths=False),
                ("Format", "Path"),
            )
            + "</div>"
        )
