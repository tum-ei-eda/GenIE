...
from .config import InvalidConfig, PassedDirectoryError
from .config.config import GenIEConfig
from .flows import GenIEFlow, SequentialGenIEFlow, FlowException, FlowError, cloup_flow_opts
...
def run(
    ...
    try:
        ...
        TargetFlow: Optional[Type[GenIEFlow]] = GenIEFlow.factory.get("Demo")

        for config_file in config_files:
            if meta := GenIEConfig.get_meta(config_file):
                # to maintain backwards compat, in 3 you will need to explicitly
                # set the flow you're substituting
                target_flow_desc = meta.flow or flow_name or "Demo"

                if isinstance(target_flow_desc, str):
                    if found := GenIEFlow.factory.get(target_flow_desc):
                        TargetFlow = found
                    else:
                        err(f"Unknown flow '{target_flow_desc}' specified.")
                        ctx.exit(1)
                elif isinstance(target_flow_desc, list):
                    TargetFlow = SequentialGenIEFlow.make(target_flow_desc)
                if meta.substituting_steps is not None and issubclass(TargetFlow, SequentialGenIEFlow):
                    if meta.flow is None:
                        warn(
                            'config_file currently has substituting_steps set with no flow, where it will fall back to Demo. Please update your configuration to explicitly set "flow" to "Demo".'
                        )
                    TargetFlow = TargetFlow.Substitute(meta.substituting_steps)  # type: ignore  # Type checker is being rowdy with this one

        if flow_name is not None:
            if found := GenIEFlow.factory.get(flow_name):
                TargetFlow = found
            else:
        ...
        assert TargetFlow is not None, "TargetFlow is unexpectedly None. Please report this as a bug."
        ...
        err("GenIE will now quit. Please check your configuration.")
        ctx.exit(1)
    except ValueError as e:
        err(e)
        debug(traceback.format_exc())
        err("GenIE will now quit.")
        ctx.exit(1)
...
def cli(ctx, /, **kwargs):
    """
    Runs a GenIE flow via the commandline using a design configuration
    object.
