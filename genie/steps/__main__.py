...
def load_step_from_inputs(
    ...
    Target = GenIEStep
    if id is not None:
        if Found := GenIEStep.factory.get(id):
            Target = Found
        else:
            err(f"No step registered with id '{id}'. Ensure all relevant plugins are installed.")
            ctx.exit(-1)
...
def run(ctx, output, state_in, config, id):
    ...
    if step.config.meta.genie_version != __version__:
        warn(
            "GenIE version being used is different from the version this step was originally run with. Procceed with caution."
        )
...
def eject(ctx, output, state_in, config, id):
    ...
    "ejects" GenIE and just returns a shell script that runs this subprocess.

    This is useful for:

    * GenIE developers and maintainers reporting issues to original tool
      developers
    * Advanced users who are sure that the issue is not GenIE-specific and
      would like to skip reporting an issue with GenIE first
    """
    ...
    if found_cmd is None:
        err("Could not eject: The step did not successfully invoke a subprocess using run_subprocess.")
        exit(-1)

    canon_scripts_dir = os.path.join(get_genie_root(), "scripts")
    ...
    if found_env is not None:
        for key, value in found_env.items():
            if value == current_env.get(key) or key in filtered_env or key in ["PATH", "PYTHONPATH"]:
                continue
...
def create_reproducible(
    ...
    * A step directory (--step-dir) generated from a previous run
    ...
    on any computer that has the appropriate version of GenIE installed
    (as well as the underlying utility for that specific step.)

    The reproducible will report an error if GenIE is not installed and will
    emit a warning if the installed version of GenIE mismatches the one
    ...
    if step_dir is None:
        if config is None or state_in is None:
            ...
            err("Both --config and --state-in must be provided if the --step-dir is not provided.")
            ctx.exit(-1)
    else:
        if config is None:
...
def cli():
    """
    Try 'python3 -m genie.steps COMMAND --help' for help with a specific
    command.
    """
    pass
