...
from openlane.state import DesignFormat, DesignFormatObject
...
class OutputProcessor(ABC, Generic[VT]):
    ...
    def __init__(self, step: GenIEStep, report_dir: str, silent: bool) -> None:
        self.step = step
...
class ProcessStatsThread(Thread):
            ...
            while status not in [psutil.STATUS_ZOMBIE, psutil.STATUS_DEAD]:
                with self.process.oneshot():
                    ...
                    for key in self.peak_resources.keys():
                        self.peak_resources[key] = max(current[key], self.peak_resources[key])

                        # moving average
                        self.avg_resources[key] = ((count * self.avg_resources[key]) + current[key]) / (count + 1)
    ...
    def stats_as_dict(self):
        return {
            ...
            "peak_resources": {
                k: (self.peak_resources[k] if "memory" not in k else format_size(int(self.peak_resources[k])))
                for k in self.peak_resources
            },
            "avg_resources": {
                k: (self.avg_resources[k] if "memory" not in k else format_size(int(self.avg_resources[k])))
...
class GenIEStep(ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if state_in is None:
            if GenIEConfig.current_interactive is not None:
                raise TypeError(
        ...
        if _no_revalidate_conf:
            ...
            # print("self.get_all_config_variables()", self.get_all_config_variables())
    ...
    def warn(self, msg: object, /, **kwargs):
        """
        Logs to the GenIE logger with the log level WARNING, appending the
    ...
    def err(self, msg: object, /, **kwargs):
        """
        Logs to the GenIE logger with the log level ERROR, appending the
        step's ID as extra data.
        ...
        *,
            ...
            for input, output in zip_longest(Self.inputs, Self.outputs):
                ...
                if output is not None:
                    if not isinstance(output, DesignFormat):
                        raise StepException(f"Output '{output}' is not a valid DesignFormat enum object.")
        ...
        if len(Self.config_vars):
            ...
            for var in Self.config_vars:
                units = var.units or ""
                result += f"| `{var.name}`{{#{var._get_docs_identifier(Self.id)}}} | {var.type_repr_md(for_document=True)} | {var.desc_repr_md()} | `{var.default}` | {units} |\n"
            result += "\n"
    ...
    def _repr_markdown_(self) -> str:  # pragma: no cover
        ...
        assert self.start_time is not None, "Start time not set even though self.state_out exists"
        assert self.end_time is not None, "End time not set even though self.state_out exists"
    ...
    @classmethod
    def _load_config_from_file(Self, config_path: Union[str, os.PathLike]) -> GenIEConfig:
    ...
    def load(
        Self,
        config: Union[str, os.PathLike, GenIEConfig],
        ...
        if Self.id == NotImplemented:  # If abstract
            id, Target = GenIEStep.factory.from_step_config(config)
            if id is None:
                raise StepNotFound(
    ...
    def load_finished(
        Self,
        step_dir: str,
        search_steps: Optional[List[Type[GenIEStep]]] = None,
    ) -> "GenIEStep":
        ...
        try:
            step_object = Self.load(config_path, state_in_path)
        except StepNotFound as e:
            if e.id is not None:
                search_steps = search_steps or []
                Matched: Optional[Type[GenIEStep]] = None
                ...
                if Matched is None:
                    raise e from None
                step_object = Matched.load(config_path, state_in_path)
            else:
                raise e from None
    ...
    def get_all_config_variables(Self) -> List[Variable]:
        variables_by_name: Dict[str, Variable] = {
            variable.name: variable for variable in universal_genie_flow_config_variables
        }
    ...
    def create_reproducible(
        ...
        """
        Creates a folder that, given a specific version of GenIE being
        ...
        dumpable_config = {k: dumpable_config[k] for k in sorted(dumpable_config)}  # sort dict
        ...
        for format in DesignFormat:
            ...
            if format not in self.__class__.inputs and not (
                ...
                and DesignFormat.ODB in self.__class__.inputs  # hack to write tests a bit more easily
            ):
        ...
        # 3. Runner (GenIE)
        script_path = os.path.join(target_dir, "run_genie.sh")
        with open(script_path, "w") as f:
            f.write(
                textwrap.dedent(
                    ...
                    #!/bin/sh
                    set -e
                    python3 -m genie --version
                    if [ "$?" != "0" ]; then
                        echo "Failed to run 'python3 -m genie --version'."
                        exit -1
                    fi
        ...
        hyperlinks = (
            os.getenv(
                "_i_want_genie_to_hyperlink_things_for_some_reason",
                None,
            )
            == "1"
        )
        ...
        info(f"Reproducible created at: {link_start}'{os.path.relpath(target_dir)}'{link_end}")
    ...
    def start(
        ...
        **kwargs,
        ...
        if step_dir is None:
            if GenIEConfig.current_interactive is not None:
                self.step_dir = os.path.join(
                    os.getcwd(),
                    "genie_run",
                    f"{GenIEStep.counter}-{slugify(self.id)}",
                )
                GenIEStep.counter += 1
            else:
        ...
        verbose(f"Running '{self.id}' at {link_start}'{os.path.relpath(self.step_dir)}'{link_end}…")
        ...
        for input in self.inputs:
            ...
            if value is None:
                raise StepException(f"{type(self).__name__}: missing required input '{input.name}'") from None

        try:
            views_updates, metrics_updates, paths_updates = self.run(state_in_result, **kwargs)
        except subprocess.CalledProcessError as e:
            if e.returncode is not None and e.returncode < 0:
                raise StepSignalled(f"{self.name}: Interrupted ({Signals(-e.returncode).name})") from None
            else:
                raise StepError(f"{self.name}: subprocess {e.args} failed", underlying_error=e) from None

        metrics = GenericImmutableDict(state_in_result.metrics, overrides=metrics_updates)
        paths = GenericImmutableDict(state_in_result.paths, overrides=paths_updates)

        self.state_out = state_in_result.__class__(
            state_in_result, overrides=views_updates, metrics=metrics, paths=paths
        )

        try:
            self.state_out.validate()
        except InvalidState as e:
            raise StepException(f"Step {self.name} generated invalid state: {e}") from None
    ...
    def run_subprocess(
        ...
        **kwargs,
        ...
        """
        A helper function for :class:`GenIEStep` objects to run subprocesses.
        ...
        :param output_processing: An override for the class's list of
            :class:`genie.steps.OutputProcessor` classes.
        :param \\*\\*kwargs: Passed on to subprocess execution: useful if you want to
            redirect stdin, stdout, etc.
        :returns: A dictionary of output processor results.
            ...
            * ``log_path``: The resolved log path for the subprocess
            ...
            and :meth:`genie.steps.OutputProcessor.result` methods of the
            output processors.
        ...
        for key, value in env.items():
            if not (isinstance(value, str) or isinstance(value, bytes) or isinstance(value, os.PathLike)):
                raise StepException(f"Environment variable for key '{key}' is of invalid type {type(value)}: {value}")
        ...
        if check and returncode != 0:
            if returncode > 0:
                ...
                if concatenated.strip() != "":
                    self.err(f"Last {len(line_buffer)} line(s):\n" + escape(concatenated))
                self.err(f"Full log file: {link_start}'{os.path.relpath(log_path)}'{link_end}")
    ...
    @classmethod
    def with_id(Self, id: str) -> Type["GenIEStep"]:
        """
    ...
    class GenIEStepFactory(object):
        ...
        __registry: ClassVar[Dict[str, Type[GenIEStep]]] = {}

        @classmethod
        def from_step_config(
            Self, step_config_path: Union[GenIEConfig, str, os.PathLike]
        ) -> Tuple[Optional[str], Optional[Type[GenIEStep]]]:
            if isinstance(step_config_path, GenIEConfig):
                step_id = GenIEConfig.meta.step
            else:
        ...
        @classmethod
        def register(Self) -> Callable[[Type[GenIEStep]], Type[GenIEStep]]:
            """
            Adds a step type to the registry using its :attr:`GenIEStep.id` attribute.
            """

            def decorator(cls: Type[GenIEStep]) -> Type[GenIEStep]:
                if cls.id == NotImplemented:
                    raise RuntimeError(f"Abstract step {cls} without property .id cannot be registered.")
        ...
        @classmethod
        def get(Self, name: str) -> Optional[Type[GenIEStep]]:
            """
            Retrieves a GenIEStep type from the registry using a lookup string.

            :param name: The registered name of the GenIEStep. Case-insensitive.
            """
    ...
    factory = GenIEStepFactory


class CompositeGenIEStep(GenIEStep):
    """
    ...
    def run(self, state_in: State, **kwargs) -> Tuple[ViewsUpdate, MetricsUpdate]:
        ...
        for i, Step in enumerate(self.Steps):
            ...
            step_dir = os.path.join(self.step_dir, f"{str(i + 1).zfill(ordinal_length)}-{slugify(step.id)}")
            state = step.start(
        ...
        for key in state:
            if state_in.get(key) != state.get(key) and DesignFormat.by_id(key) in self.outputs:
