...
from .flow import FlowException, FlowError, FlowProgressBar
from ..config import GenIEConfig, AnyConfigs, Variable
from ..config import universal_genie_flow_config_variables
...
Substitution = Union[str, Type[GenIEStep], None]


class SequentialGenIEFlow(GenIEFlow):
    """
    ...
    Substitutions: Optional[Dict[str, Union[str, Type[GenIEStep], None]]] = None
    ...
    def __init__(
        ...
        *,
        ...
        Substitute: Optional[Dict[str, Union[str, Type[GenIEStep], None]]] = None,
    ):
        ...
        for id, variable_names in Self.gating_config_vars.items():
            ...
            for var_name in variable_names:
                ...
                if variables_by_name[var_name].type != bool:
                    raise TypeError(f"Gating variable '{var_name}' in Flow '{Self.__qualname__}' is not a Boolean")
    ...
    @classmethod
    def make(Self, step_ids: List[str]) -> Type[SequentialGenIEFlow]:
        Step_list = []
        for name in step_ids:
            step = GenIEStep.factory.get(name)
            if step is None:
                raise TypeError(f"No step found with id '{name}'")
            Step_list.append(step)

        class CustomSequentialGenIEFlow(SequentialGenIEFlow):
            name = "Custom Sequential Flow"
            Steps = Step_list

        return CustomSequentialGenIEFlow

    @classmethod
    def Substitute(Self, Substitutions: Dict[str, Substitution]) -> Type[SequentialGenIEFlow]:
        """
    ...
    def __substitute_step(
        target: Union[SequentialGenIEFlow, Type[SequentialGenIEFlow]],
        id: str,
        with_step: Union[str, Type[GenIEStep], None],
    ):
        ...
        for i, step in enumerate(target.Steps):
            if (
                step.id != NotImplemented  # Will be validated later by initialization: ignore for now
                and fnmatch.fnmatch(step.id.lower(), id.lower())
            ):
                step_indices.append(i)
        if len(step_indices) == 0:
            if with_step is None:
                raise FlowException(f"Could not remove '{id}': no steps with ID '{id}' found in flow")
            raise FlowException(f"Could not {mode} '{id}' with '{with_step}': no steps with ID '{id}' found in flow.")
        ...
        if isinstance(with_step, str):
            with_step_opt = GenIEStep.factory.get(with_step)
    ...
    @staticmethod
    def __normalize_step_ids(target: Union[SequentialGenIEFlow, Type[SequentialGenIEFlow]]):
        ids_used: Set[str] = set()

        for i, step in enumerate(target.Steps):
            counter = 0
            id = step.id
            if id == NotImplemented:  # Will be validated later by initialization: ignore for now
                continue
