...
from ..config import GenIEConfig, Variable, universal_genie_flow_config_variables, AnyConfigs
from ..state import State
from ..steps import GenIEStep, StepNotFound
from ..logging import (
    LevelFilter,
    console,
    info,
...
def ensure_progress_started(method: T) -> Callable:
    ...
    def _impl(obj: FlowProgressBar, *method_args, **method_kwargs):
        if not obj.started:
            raise FlowException(f"Attempted to call method '{method}' before initializing progress bar")
        return method(obj, *method_args, **method_kwargs)

    if method.__doc__ is None:
        method.__doc__ = ""

    method.__doc__ = "This method may not be called before the progress bar is started.\n" + method.__doc__
...
class GenIEFlow(ABC):
    ...
    :param config: Either a resolved :class:`genie.config.GenIEConfig` object, or an
        input to :meth:`genie.config.GenIEConfig.load`.
    ...
    :param config_override_strings: See :meth:`genie.config.GenIEConfig.load`
    :param design_dir: See :meth:`genie.config.GenIEConfig.load`
    ...
    class _StepWarningHandler(logging.Handler):
        ...
        def __init__(self, *args, **kwargs) -> None:
            ...
            self.warnings: Dict[str, GenIEFlow._StepWarningHandler.Record] = {}
        ...
        def emit(self, record: logging.LogRecord) -> None:
            ...
            if key in self.warnings:
                ...
                self.warnings[key] = GenIEFlow._StepWarningHandler.Record(record.msg, step)

    name: str = NotImplemented
    Steps: List[Type[GenIEStep]] = NotImplemented  # Override
    config_vars: List[Variable] = []
    step_objects: Optional[List[GenIEStep]] = None
    ...
    def __init__(
        ...
        *,
        ...
        for step in self.Steps:
            step.assert_concrete("used in a GenIEFlow")

        self.name = self.__class__.__name__ if self.name == NotImplemented else self.name
        if name is not None:
            self.name = name
        ...
        if not isinstance(config, GenIEConfig):
            config, design_dir = GenIEConfig.load(
                config_in=config,
    ...
    def get_help_md(Self) -> str:  # pragma: no cover
        ...
        result = (
            textwrap.dedent(
                ...
                ```sh
                genie --flow {Self.__name__} [...]
                ```

                #### Importing

                ```python
                from genie.flows import GenIEFlow

                {Self.__name__} = GenIEFlow.factory.get("{Self.__name__}")
                ```
                """
            )
            % doc_string
        ...
        if len(flow_config_vars):
            ...
            for var in flow_config_vars:
                units = var.units or ""
                result += f"| `{var.name}`{{#{var._get_docs_identifier(Self.__name__)}}} | {var.type_repr_md()} | {var.desc_repr_md()} | `{var.default}` | {units} |\n"
            result += "\n"
    ...
    def get_all_config_variables(self) -> List[Variable]:
        ...
        flow_variables_by_name: Dict[str, Tuple[Variable, str]] = {
            variable.name: (variable, "universal flow variables") for variable in universal_genie_flow_config_variables
        }
        ...
        for step_cls in self.Steps:
            for variable in step_cls.config_vars:
                if flow_variables_by_name.get(variable.name) is not None:
                    existing_variable, existing_step = flow_variables_by_name[variable.name]
                    if variable != existing_variable:
                        raise FlowException(
    ...
    def init_with_config(
        Self,
        config_in: Union[GenIEConfig, str, os.PathLike, Dict],
        **kwargs,
    ):  # pragma: no cover
    ...
    def start(
        ...
        **kwargs,
        ...
        warning_handler = GenIEFlow._StepWarningHandler()
        ...
        self.run_dir = os.path.abspath(_force_run_dir or os.path.join(self.design_dir, "runs", tag))
        ...
        try:
                ...
                if not _no_load_previous_steps:
                    try:
                        self.step_objects.append(
                            GenIEStep.load_finished(
                                os.path.join(self.run_dir, entry),
                                self.Steps,
                            )
                        )
                    except StepNotFound as e:
                        raise FlowException(f"Error while loading concluded step in {entry}: {e}")
            ...
            if with_initial_state is None:
                if latest_json := get_latest_file(self.run_dir, "state_out.json"):
                    ...
                    initial_state = State.loads(open(latest_json, encoding="utf8").read())

        except NotADirectoryError:
            raise FlowException(f"Run directory for '{tag}' already exists as a file and not a directory.")
        except FileNotFoundError:
        ...
        try:
            ...
            # print("self.config_resolved_path", self.config_resolved_path)
            with open(self.config_resolved_path, "w") as f:
                f.write(self.config.dumps())

            self.progress_bar = FlowProgressBar(self.name, starting_ordinal=starting_ordinal)
            self.progress_bar.start()
            # print("initial_state", initial_state)
            # print("starting_ordinal", starting_ordinal)
            # print("kwargs", kwargs)
    ...
    @protected
    def dir_for_step(self, step: GenIEStep) -> str:
        """
        ...
        if self.run_dir is None:
            raise FlowException("Attempted to call dir_for_step on a flow that has not been started.")
        return os.path.join(
            self.run_dir,
    ...
    def start_step(
        ...
        **kwargs,
        ...
        """
        A helper function that handles passing parameters to :mod:`GenIEStep.start`.'
        ...
        See :meth:`GenIEStep.start` for more info.
    ...
    def start_step_async(
        ...
        **kwargs,
        ...
        :returns: A ``Future`` encapsulating a State object, which can be used
            ...
            ``Future`` to be realized before calling :meth:`GenIEStep.run`)
        """
    ...
    #     if self.step_objects is None or self.toolbox is None or self.config_resolved_path is None:
    #         raise RuntimeError("Flow was not run before attempting to save views in the Efabless format.")
    ...
    #     # 1. Copy Files
    #     last_state.validate()
    #     info(f"Saving views in the Efabless/Caravel User Project format to '{os.path.abspath(path)}'…")
    #     mkdirp(path)
    ...
    #                 assert self.toolbox is not None, "toolbox check was not executed properly"
    ...
    #                     target_path = os.path.join(default_corner_target_dir, target_basename)
    #                     shutil.copyfile(default_corner_view[0], target_path, follow_symlinks=True)
    #                 else:
    ...
    #                 shutil.copyfile(file_path, os.path.join(to_dir, file), follow_symlinks=True)

    #     signoff_folder = os.path.join(path, "signoff", self.config["DESIGN_NAME"], "genie-signoff")
    ...
    #             if os.path.exists(reports_dir):
    #                 copy_dir_contents(reports_dir, signoff_folder, "antenna_summary.rpt")
    ...
    class FlowFactory(object):
        ...
        __registry: ClassVar[Dict[str, Type[GenIEFlow]]] = {}

        @classmethod
        def register(Self, registered_name: Optional[str] = None) -> Callable[[Type[GenIEFlow]], Type[GenIEFlow]]:
            """
            ...
            def decorator(cls: Type[GenIEFlow]) -> Type[GenIEFlow]:
                name = cls.__name__
        ...
        @classmethod
        def get(Self, name: str) -> Optional[Type[GenIEFlow]]:
            """
            Retrieves a GenIEFlow type from the registry using a lookup string.
